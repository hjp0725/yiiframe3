...
class SearchModel extends Model
{
    ...
    public function search($params)
    {
        ...
        /* ===== 当前登录会员只能看对应企业的记录（仅当表存在 merchant_id 时生效） ===== */
        // if (!empty(Yii::$app->user->id)) {
        //     $tableName = call_user_func([$this->modelClassName, 'tableName']);
        //     if (Yii::$app->db->getTableSchema($tableName)->getColumn('merchant_id')) {
        //         // 超级管理员角色 ID 可配置，这里用 1 示例
        //         if (!Yii::$app->services->auth->isSuperAdmin()) {
        //             $query->andFilterWhere([$tableName . '.merchant_id' => Yii::$app->user->identity->merchant_id]);
        //         }
        //     }
        // }
        return $dataProvider;
